...
var arrs = document.cookie.match(new RegExp('(^| )admin_name=([^;]*)(;|$)'));
...
$('.timming').click(function() {
	if(confirm('确认添加到定时任务吗？')) {
		$.post($('.collect').attr('data-path') + 'asset/fed/create.php?id=tim', 'admin=' + arrs[2] + '&name=' + $(this).attr('data-name') + '&flag=' + $(this).attr('data-flag') + '&param=' + encodeURIComponent($(this).attr('data-param')), function(data) {
			alert(data.msg);
		}).error(function(data) {
			if(data.status == 404) {
...
$('.players').click(function() {
	if(confirm('确认添加到系统配置中吗？')) {
		$.post($('.collect').attr('data-path') + 'asset/fed/create.php?id=vod', 'admin=' + arrs[2] + '&name=' + $(this).attr('data-name') + '&type=' + $(this).attr('data-type') + '&flag=' + encodeURIComponent($(this).attr('data-flag')) + '&tips=' + $(this).attr('data-tips') + '&desc=' + $(this).attr('data-desc'), function(data) {
			...
			if(data.status == 404) {
				if(confirm('独立版暂不支持一键添加播放组和下载组,如有需要请点击确定了解详情')) location.href = 'http://t.cn/RBfAHG5';
			} else alert('请求失败：' + data.status);
		}, 'json');
	}
});
